...
void *thread_main(void *arg_in) {
    ...
    int max = gmn_get_max();
    assert(!send_all(sockfd, (void*)&max, sizeof(MAX_VALUE)));
    
    int guess, result;
    do {
        assert(!recv_all(sockfd, (void*)&guess, sizeof(guess)));
        result = gmn_check(&gmn, guess);
        assert(!send_all(sockfd, (void*)&result, sizeof(result)));
        if (result == 0) {
            char *msg = gmn_get_message(&gmn);
            int msg_len = strlen(msg);
            assert(!send_all(sockfd, (void*)&msg_len, sizeof(msg_len)));
            assert(!send_all(sockfd, msg, msg_len));
        }
    } while (result != 0);
...
int main(void) {
    ...
    // me when no `if let Err(rv) = getaddrinfo(...)` :(
    ...
    while (1) {
        ...
        if (create_handler(new_fd, r) < 0) close(new_fd);
    }
