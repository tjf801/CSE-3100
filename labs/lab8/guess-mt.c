...
void * thread_c(void * arg_in) {
    ...
    assert(!pthread_mutex_lock(&arg->mutex));
    
    // send max value
    arg->max = gmn_get_max();
    arg->status = S_INIT;
    assert(!pthread_cond_signal(&arg->cond_result));
    
    // repeat the following until guess is correct
    do {
        // wait for a guess from thread_p
        while (arg->status != S_GUESS)
            assert(!pthread_cond_wait(&arg->cond_guess, &arg->mutex));
        ...
        // send the result and, if the guess is correct, the final message to thread_p
        arg->status = S_RESULT;
        if (arg->result == 0) strncpy(arg->message, gmn_get_message(&gmn), MSG_BUF_SIZE);
        assert(!pthread_cond_signal(&arg->cond_result));
    } while (arg->result != 0);
    
    assert(!pthread_mutex_unlock(&arg->mutex));
...
void *thread_p(void *arg_in) {
    ...
    // get max from thread_c
    assert(!pthread_mutex_lock(&arg->mutex));
    while (arg->status != S_INIT)
        assert(!pthread_cond_wait(&arg->cond_result, &arg->mutex));
    max = arg->max;
    ...
    do {
        ...
        arg->guess = guess;
        arg->status = S_GUESS;
        assert(!pthread_cond_signal(&arg->cond_guess));
