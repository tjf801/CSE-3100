...
double cum_prob(const int k, const unsigned long long trials) {
	unsigned long long successes = 0;
	
	for (__uint64_t i = 0; i < trials; ++i) {
		const unsigned int d1 = rand() % 6 + 1;
		const unsigned int d2 = rand() % 6 + 1;
		const unsigned int d3 = rand() % 6 + 1;
		
		if (d1 + d2 + d3 >= k) successes++;
	}
