...
void init_array(int n, unsigned long a[static n]) {   
    a[0] = 1;
    for (int i = 1; i < n; i++)
        a[i] = a[i-1] + i + 1;
}
...
unsigned long first_digit(unsigned long k) {
    while (k >= DIGITS)
	    k = k / DIGITS;
    return k;
}
...
void frequencies(int n, const unsigned long a[static n], unsigned long freq[DIGITS]) {
    for (int k = 0; k < DIGITS; k++)
        freq[k] = 0;
    
    for (int i = 0; i < n; i++)
        freq[first_digit(a[i])]++;
}

void* thread_freq(void* threadarg) {
    struct thread_data* my_data = (struct thread_data*) threadarg;
    frequencies(my_data->j - my_data-> i + 1, &a[my_data->i], my_data->freq);
	pthread_exit(NULL);
}
