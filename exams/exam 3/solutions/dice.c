...
void *producer(void *t) {
    ...
    while (!done) {
        ...
        pdata->data = v;
        pthread_cond_broadcast(&pdata->cond[v - 1]);
        while (pdata->ready)
...
void *consumer(void *t) {
    ...
    while (!done) {
        //TODO:
        //Fill in code below
        while (!pdata->ready || (pdata->data != id + 1 && pdata->data != -1))
            pthread_cond_wait(&pdata->cond[id], &pdata->mutex);
        if (pdata->data == -1) break;
        assert(pdata->data == id + 1);
        (*p_count)++;
        pdata->ready = 0;
        pthread_cond_broadcast(&pdata->cond[0]);
    }
