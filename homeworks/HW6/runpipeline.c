...
void check_pointer(void *p) {
    // why even have this function? real sigmas just dereference the pointer and segfault smh my head
    if (p == NULL)
       die("Pointer is NULL.");
}
...
void start_program(Program *programs, int num_programs, int cur) {
    if (cur >= num_programs) die("invalid array index");
    
    Program *program_to_start = &programs[cur];
    ...
    if (pid == 0) {
        // redirect stdin
        if (program_to_start->fd_in != FD_STDIN) {
            dup2(program_to_start->fd_in, FD_STDIN);
            close(program_to_start->fd_in);
        }
        
        // redirect stdout
        if (program_to_start->fd_out != FD_STDOUT) {
            dup2(program_to_start->fd_out, FD_STDOUT);
            close(program_to_start->fd_out);
        }
        
        // execute
        execvp(program_to_start->argv[0], program_to_start->argv);
        
        // if we are here, exec failed
        die("execvp() failed");
    }
    
    program_to_start->pid = pid;
    if (program_to_start->fd_in != FD_STDIN) close(program_to_start->fd_in);
    if (program_to_start->fd_out != FD_STDOUT) close(program_to_start->fd_out);
}
...
void prepare_pipes(Program *programs, int num_programs) {
    ...
    for (int i = 0; i < num_programs - 1; ++i) {
        int _pipe[2];
        if (pipe(_pipe) == -1) die("pipe failed");
        
        programs[i].fd_out = _pipe[PIPEFD_WRITE];
        programs[i+1].fd_in = _pipe[PIPEFD_READ];
    }
